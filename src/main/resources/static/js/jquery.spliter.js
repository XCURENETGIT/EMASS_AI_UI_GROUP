...
(function($, undefined) {
	...
	$.fn.spliter = function(options) {
		...
		var self = $.extend(this, {
					...
					return function(n, silent) {
						if (n === undefined) {
							...
							if (settings.invisible) {
								...
								if((pw+self.limit) >= self.width()) {
									pw = panel_1.width((self.width()-self.limit) - sw2).outerWidth();
								}
			...
			destroy : function() {
				$(target.selector).unbind('sizeChanged');
				$(target.selector).off('sizeChanged');
				...
				panel_1.css({'width':'', 'height':''});
				panel_2.css({'width':'', 'height':''});
		...
		if (splitters.length == 0) { // first time bind events to document
			...
			$(target.selector).sizeChanged(function(element){
                $.each(splitters, function(i, splitter) {
					splitter.refresh();
				});
            });
			...
			$(document.documentElement).bind('mousedown.splitter', function(e) {
				if (splitter_id !== null && e.which == 1) {
					...
					$(panel_1).css({pointerEvents:'none', 'user-select':'none'});
					$(panel_2).css({pointerEvents:'none', 'user-select':'none'});
					return false;
				}
				...
				if (current_splitter) {
					...
					$(panel_1).css({pointerEvents:'', 'user-select':''});
					$(panel_2).css({pointerEvents:'', 'user-select':''});
				}
