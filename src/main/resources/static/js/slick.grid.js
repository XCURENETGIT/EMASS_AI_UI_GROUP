...
(function ($) {
   ...
   **/
  function SlickGrid(target, container, data, columns, options) {
    ...
    function init() {
      ...
      $viewport = $("<div class='slick-viewport scrollbar' style='width:100%;overflow:auto;outline:0;position:relative;;'>").appendTo($container);
    ...
    function handleScroll() {
      ...
      if (vScrollDist) {
        ...
        if ( scrollTop + $viewport.innerHeight() >= $viewport[0].scrollHeight ) {
        	trigger(self.nextPage, {scrollLeft: scrollLeft, scrollTop: scrollTop});
        }
      }
    ...
    function handleClick(e) {
    	if(e.which!=1) return; //왼쪽 마우스만 작동 되도록 처리.
      if (!currentEditor) {
...
/**
 * slick.cellexternalcopymanager.js
 */
(function ($) {
	  // register namespace
	  $.extend(true, window, {
	        ...
	        if (e.which == keyCodes.C && (e.ctrlKey || e.metaKey)) {    // CTRL + C
	          ...
	          if (ranges.length != 0) {
	            ...
	            for (var rg = 0; rg < ranges.length; rg++){
	                ...
	                // in cases of multiple ranges and each has a single array, then the join won't add the newlines so force it
	                // this happens when using ctrl-A to select all
	                if(clipTextRows.length == 1 && rg < ranges.length-1) { clipTextRows[0] += "\r\n"; }
