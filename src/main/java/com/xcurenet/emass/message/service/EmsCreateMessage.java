...
public class EmsCreateMessage {
	...
	public EmsCreateMessage(HttpServletRequest request) {
		...
		infoHynixConf = Config.getBoolean("info.hynix.used");
	}
	...
	public String getHeaderMessage(String msgId, String body, String print, Locale locale, String firstAdminYn, MessengerGroupUserVO participants, String adminId, String adminType) {
		...
		try {
			is = getClass().getResourceAsStream("../../../files/headerTable.html");
			in = new BufferedReader(new InputStreamReader(is, Common.UTF8));
	...
	/*	try {
			...
			if( participants == null){
				if (Common.isEquals(msg.getKwd(), "Y")) {
					...
					if (attachStr.length() > 0) attachStr = attachStr.substring(0, attachStr.length() - 2);
					if (fileNameStr.length() > 0) fileNameStr = fileNameStr.substring(0, fileNameStr.length() - 2);
					if (subjectStr.length() > 0) subjectStr = subjectStr.substring(0, subjectStr.length() - 2);
			...
			for (int i = 0; i < user.size(); i++) {
				EmsRecvVO u = user.get(i);
				userStr += makeUserHtml(u, formatval);
			}

			for (int i = 0; i < sender.size(); i++) {
				EmsRecvVO u = sender.get(i);
				senderStr += makeUserHtml(u, formatval);
			}

			for (int i = 0; i < to.size(); i++) {
				EmsRecvVO u = to.get(i);
				toStr += makeUserHtml(u, formatval);
				toStr += "; ";
			}

			for (int i = 0; i < cc.size(); i++) {
				EmsRecvVO u = cc.get(i);
				ccStr += makeUserHtml(u, formatval);
				ccStr += "; ";
			}

			for (int i = 0; i < bcc.size(); i++) {
				EmsRecvVO u = bcc.get(i);
				bccStr += makeUserHtml(u, formatval);
				bccStr += "; ";
			}
			...
			String[] vals = {Prop.propFormat("OPERATION_MGMT.BODY_VIEW", locale), Prop.propFormat("condition.subject", locale)+" "+Prop.propFormat("bodyview.find.keyword", locale),
						...
						Common.nvl(msg.getUsrId()), userStr, senderStr, toStr, ccStr, bccStr, Common.nvl(ipBusiNm),Common.nvl(ipDeptNm), Common.nvl(msg.getHost())+Common.nvl(msg.getPath())+Common.nvl(msg.getQuery()), attachStr, fileNameStr, getFileHtml(files), bodyStr, getPatternHtml(pattern, locale),
			...
			while ((tmp = in.readLine()) != null) {
				for (int i = 0; i < keys.length; i++) {
					if (tmp.indexOf(keys[i]) > -1) {
						String start = tmp.substring(0, tmp.indexOf(keys[i]));
						String end = tmp.substring(tmp.indexOf(keys[i]) + keys[i].length(), tmp.length());
			...
			if (to.size() == 0) header_doc.getElementById("msgTo").remove();
			if (cc.size() == 0) header_doc.getElementById("msgCc").remove();
			if (bcc.size() == 0) header_doc.getElementById("msgBcc").remove();
			...
			if (files.size() == 0) header_doc.getElementById("msgFiles").remove();
			else {
			...
			if (pattern.size() == 0) header_doc.getElementById("msgPattern").remove();
	...
	private String getSkMlConfdProbPercent(double ml_confd_prob) {
		if( ml_confd_prob == -1 ) return "";
		else return "(" + Math.floor(ml_confd_prob * 100) + "%)";

	}

	private String getSkMlConfdFeedbackStr(int ml_confd_feedback, Locale locale) {
		String str = "";
		if(ml_confd_feedback == 1) str += Prop.propFormat("condition.info.secretFeedbackY", locale);
		else if(ml_confd_feedback == 9) str += Prop.propFormat("condition.info.feedback9", locale);
		else if(ml_confd_feedback == 0) str += Prop.propFormat("condition.info.secretFeedbackN", locale);
		return str;
	}
	...
	private String getFileHtml(List<EmsAttachVO> files) {
		...
		for (int i = 0; i < files.size(); i++) {
			...
			resultStr += "	</tr> ";
			if(infoHynixConf) {
				if (file.getMl_confd_class() == 1) {
					resultStr = resultStr + Prop.propFormat("condition.info.Y", locale);
				} else if (file.getMl_confd_class() == 0) {
					resultStr = resultStr + Prop.propFormat("condition.info.N", locale);
				} else {
					resultStr = resultStr + Prop.propFormat("common.msg.noinfo", locale);
				}

				if (file.getMl_confd_prob() == -1.0D) return "";
				else return "(" + Math.floor(file.getMl_confd_prob() * 100.0D) + "%)";
			}
		}
		resultStr += "</table> ";
		return resultStr;
