...
menuArr.push({id:'DASHBOARD_MENU',		name:'<s:message code="DATA_MONITOR.DASHBOARD_MENU"/>',  		p_id:'DATA_MONITOR'}); //대시보드 메뉴
menuArr.push({id:'DASHBOARD_SETUP',		name:'<s:message code="DATA_MONITOR.DASHBOARD_SETUP"/>',  		p_id:'DATA_MONITOR'}); //대시보드 관리
menuArr.push({id:'MESSAGE_INFO', 		name:'<s:message code="DATA_MONITOR.MESSAGE_INFO"/>',  			p_id:'DATA_MONITOR'}); //메시지 정보
menuArr.push({id:'MESSAGE_SERVICE',		name:'<s:message code="DATA_MONITOR.MESSAGE_SERVICE"/>',  		p_id:'DATA_MONITOR'}); //메신저 모아보기
menuArr.push({id:'GENERATIVEAI_SERVICE',		name:'<s:message code="DATA_MONITOR.GENERATIVEAI_SERVICE"/>',  		p_id:'DATA_MONITOR'}); //생성형AI 모아보기
menuArr.push({id:'NOTE_SERVICE',		name:'<s:message code="DATA_MONITOR.NOTE_SERVICE"/>',  		    p_id:'DATA_MONITOR'}); //노트 모아보기
menuArr.push({id:'FILETRANSFER_SERVICE',		name:'<s:message code="DATA_MONITOR.FILETRANSFER_SERVICE"/>',  		p_id:'DATA_MONITOR'}); //파일전송 모아보기
menuArr.push({id:'INTEREST_USER', 		name:'<s:message code="DATA_MONITOR.INTEREST_USER"/>',  		p_id:'DATA_MONITOR'}); //관심 사용자 관리
menuArr.push({id:'STAT_REPORT', 		name:'<s:message code="DATA_MONITOR.STAT_REPORT"/>',  			p_id:'DATA_MONITOR'}); //리포트
menuArr.push({id:'RESERVATION_ALARM', 	name:'<s:message code="DATA_MONITOR.RESERVATION_ALARM"/>',  	p_id:'DATA_MONITOR'}); //예약 알림
...
menuArr.push({id:'REGEX_PATTERN', 	name:'<s:message code="DATA_MONITOR.REGEX_PATTERN"/>',  	p_id:'DATA_MONITOR'}); //정규식 패턴 관리
...
menuArr.push({id:'STAT_ATTACHTYPE', 	name:'<s:message code="DATA_ANALYSIS.STAT_ATTACHTYPE"/>',  		p_id:'DATA_ANALYSIS'}); //첨부파일 통계
menuArr.push({id:'STAT_ATTACHNAME', 	name:'<s:message code="DATA_ANALYSIS.STAT_ATTACHNAME"/>', 		p_id:'DATA_ANALYSIS'}); //첨부파일명 통계
menuArr.push({id:'STAT_URL', 			name:'<s:message code="DATA_ANALYSIS.STAT_URL"/>',  			p_id:'DATA_ANALYSIS'}); //URL 통계
menuArr.push({id:'STAT_ADMINREAD', 		name:'<s:message code="DATA_ANALYSIS.STAT_ADMINREAD"/>',  		p_id:'DATA_ANALYSIS'}); //운용자 열람 통계
menuArr.push({id:'STAT_DEVTRAFFIC', 	name:'<s:message code="DATA_ANALYSIS.STAT_DEVTRAFFIC"/>',  		p_id:'DATA_ANALYSIS'}); //장비 트래픽 통계
...
menuArr.push({id:'DEV_EVENTLOG',		name:'<s:message code="OPERATION_MGMT.DEV_EVENTLOG"/>',  		p_id:'OPERATION_MGMT'}); //장비 이벤트 로그
...
menuArr.push({id:'USER_GROUP_MGMT', 	name:'<s:message code="POLICY_SETUP.USER_GROUP_MGMT"/>',  	p_id:'OPERATION_MGMT'}); //사용자 그룹
menuArr.push({id:'BUSI_IPRANGE', 		name:'<s:message code="POLICY_SETUP.BUSI_IPRANGE"/>',  		p_id:'OPERATION_MGMT'}); //사업장 내부 IP 설정
menuArr.push({id:'DEPT_IPRANGE', 		name:'<s:message code="POLICY_SETUP.DEPT_IPRANGE"/>',  		p_id:'OPERATION_MGMT'}); //사업장 내부 IP 설정
...
menuArr.push({id:'HOLIDAY_BUSI', 		name:'<s:message code="OPERATION_MGMT.HOLIDAY_LABEL"/>',  		p_id:'OPERATION_MGMT'}); //업무/휴일 설정
...
$(document).ready(function(){
	...
	$("#searchStr").keypress(function(e){if( e.keyCode == 13) getData( );});
			...
			<div class="content_body">
						...
						<div class="form-group form-inline not-dashed">
							...
							<div class="form-group">
								<div class="input-group">
									<input type="text" class="form-control input-sm" placeholder="<s:message code="common.msg.searchMsg"/>" id="searchStr" style="width: 300px;">
				...
				<div class="row top_space">
					<div class="col-xs-12">
						<div class="form-group form-inline not-dashed">
							<div class="form-group">
								<select class="form-control input-sm" id="adminId" style="width: 205px;">
									<option value="">- <s:message code="auditLog.select.admin"/> -</option>
								</select>
							</div>
							<div class="form-group">
								<select class="form-control input-sm" id="pMenuId" style="width: 200px;">
									<option value="">- <s:message code="auditLog.select.pmenu"/> -</option>
								</select>
							</div>
							<div class="form-group">
								<select class="form-control input-sm" id="menuId" style="width: 200px;">
