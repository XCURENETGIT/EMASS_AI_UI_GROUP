...
<%
	...
	String getType = Common.nvl(request.getParameter("type"));
	
	String getSrcip = Common.nvl(request.getParameter("srcip"));
	String getDstip = Common.nvl(request.getParameter("dstip"));
	String getHost = Common.nvl(request.getParameter("host"));
	String getDate = Common.nvl(request.getParameter("date"));
	getDate=Common.plusDaysF(getDate,-1);
...
<html lang="ko">
<head>
<title>EMASS AI - 모니터링 실행 결과</title>
<style>
.modal-body {
	padding-top: 5px;
}
...
$(document).ready(function(){
	...
	for(var i=0; i<pattern.length; i++){
		if(host.indexOf(pattern[i]) > -1){
			var chPattern = '\\';
			chPattern += pattern[i];	
	 		host= host.replaceAll(pattern[i], chPattern);
		}
	}
	
	query = '+ltime:[' + date + '000000 TO ' + date +'235959]';
	query += ' +srcip:"' + srcIp + '" +dstip:"' + dstIp + '" +(host:"' + host + '" host_str:"' + host + '") -svc:(E* X*)';
	filterValData = {"filterName":"","filter_seq":"","p_filter_seq":"","filterType":"Q","conditions":[{"query":query,"sort":"size desc"}]};
 	getList();
});
...
<body class="mini-navbar msgBody">
	<header class="header">
		<div class="naviBack">
			...
			<span class="navi"><span id="code_title"></span><s:message code="monitor.result"/></span>
		</div>
	</header>
			...
			<div class="content_body">
				<div class="row xcn_full top_space">
					<div class="col-xs-12" style="height: 100%;">
						<div id="monitorListGrid" class="slickGrid gridArea"></div>
					</div>
				</div>
			</div>
		</div>
	...
	<%} %>
	<script type="text/javascript">
		var grid = new Xgrid('monitorListGrid', contextRoot);
		grid.onCheckBox();
		grid.autoNumber();
