...
$(document).ready(function(){
	...
	$('.print_stat').click(function() {
		...
		if(gridDetail != undefined) {
			...
			gridDetail.print('<s:message code="stat.detail.keyword.list"/>', pMenuId, menuId);
		} else {
			if (grid1.Rows == 0) {
				alert('<s:message code="common.msg.nodata"/>');
				return;
	...
	$('.excel_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			excelDownLoad(gridDetail,'<s:message code="stat.detail.keyword.list"/>');
		} else {
	...
	$('.cell_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			cellDownLoad(gridDetail,'<s:message code="stat.detail.keyword.list"/>');
		} else {
	...
	$('.pdf_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			pdfDownLoad(gridDetail,'<s:message code="stat.detail.keyword.list"/>');
		} else {
	...
	$('.csv_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			csvDownLoad(gridDetail,'<s:message code="stat.detail.keyword.list"/>');
		} else {
...
function queryMakePop(  ){
	var url    = '<c:url value="/commons/queryMake.do?statType=keyword"/>';
...
<body class="mini-navbar">
	...
	<script type="text/javascript">
		...
		grid1.loadExportMenu('<s:message code="DATA_ANALYSIS.STAT_KWD"/>');
		grid1.loadPageSize();
