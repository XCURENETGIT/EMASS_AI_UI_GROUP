	...
	<div class="searchArea w100">
		<div class="searchSub w100">
			...
			<div>
				...
				<option value="sum" selected><s:message code="condition.regexp.cnt"/></option>
				<%-- 문서 건수 --%>
				<option value="pattern"><s:message code="condition.sum.cnt"/></option>
				<%-- 총 패턴 검출--%>
				</select>
			</div>
			<div>
				<select id="piCount" name="piCount">
					...
					<option value="1" name="oneVal" selected>1<s:message code="condition.infoStat.cnt.more"/></option>
...
<script type="text/javascript">
	...
	grid2.colAdd('subject', '<s:message code="condition.subject"/>', 410, 'left', false, 'nomal', function (row, cell, value, columnDef, dataContext) {
		...
		var rtnVal = '<span title="' + body_snippet + '" onclick="" class="subject_read' + grid2.getValue(row, 'readYn') + '">' + value + '</span>&nbsp;<a href="javascript:void(0);" onclick="viewer_open(' + row + ')" class="glyphicon glyphicon-new-window new-window"></a>';
	...
	function getNodeByPI(data) {
		var result = [];
		for (var i = 0; i < data.length; i++) {
			if (data[i] && data[i]['piMap']) {
				for (var x = 0; x < piArr.length; x++) {
					if (!data[i]['piMap'].hasOwnProperty(piArr[x])) {
						data[i]['piMap'][piArr[x]] = 0;
					}
					if (data[i]['piMap'][piArr[x]] > 0) {
						result.push(piArr[x]);
					}
				}
			}
	...
	function getInfoDetailList(flag, value, type) {
		...
		ui.postJson({
			...
			userStr: userStr,
			success: function (data, total) {
                makeNetwork(data, type, data.length);
                if (grid1.Col === 2) {
	...
	function makeNetwork(data, type, pi_total) {
		var nodes = [];
		var edges = [];
		...
		if (pi_total === 0) {
			...
			for (var i = 0; i < nodeLv1.length; i++) {
				for (var j = 0; j < nodeLv2.length; j++) {
					...
					for (var x = 0; x < data.length; x++) {
						if (nodeLv1[i].id === data[x].userkey) {
							if (data[x]['piMap'] && data[x]['piMap'].hasOwnProperty(nodeLv2[j]) && data[x]['piMap'][nodeLv2[j]] > 0) {
								sum++;
							}
						}
					}
			...
			for (var i = 0; i < nodeLv2.length; i++) {
				for (var j = 0; j < nodeLv3.length; j++) {
					var sum = 0;
					for (var x = 0; x < data.length; x++) {
						if (data[x]['piMap'] && data[x]['piMap'].hasOwnProperty(nodeLv2[i]) && data[x]['piMap'][nodeLv2[i]] > 0 && data[x].ctime_yyyymmdd === nodeLv3[j]) {
							sum++;
						}
					}
					...
					for (var x = 0; x < data.length; x++) {
						...
						if (data[x].ctime_yyyymmdd === nodeLv3[i] && id === nodeLv4[j]) {
							for (var y = 0; y < nodeLv2.length; y++) {
								if (data[x]['piMap'] && data[x]['piMap'][nodeLv2[y]] > 0) {
									sum++;
								}
							}
