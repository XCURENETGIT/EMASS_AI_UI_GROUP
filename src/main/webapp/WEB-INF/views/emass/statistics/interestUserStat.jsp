...
$(document).ready(function(){
	...
	$('.print_stat').click(function() {
		...
		if(gridDetail != undefined) {
			...
			gridDetail.print('<s:message code="stat.detail.interest.list"/>', pMenuId, menuId);
		} else {
			if (grid1.Rows == 0) {
				alert('<s:message code="common.msg.nodata"/>');
				return;
			}
			grid1.print('<s:message code="DATA_MONITOR.STAT_INTEREST"/>', pMenuId, menuId);
		}
	});
	...
	$('.excel_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			excelDownLoad(gridDetail,'<s:message code="stat.detail.interest.list"/>');
		} else {
			...
			excelDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_INTEREST"/>', svg);
		}
	});
	...
	$('.cell_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			cellDownLoad(gridDetail,'<s:message code="stat.detail.interest.list"/>');
		} else {
			cellDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_INTEREST"/>');
		}
	});
	...
	$('.pdf_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			pdfDownLoad(gridDetail,'<s:message code="stat.detail.interest.list"/>');
		} else {
			pdfDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_INTEREST"/>');
		}
	});
	...
	$('.csv_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			csvDownLoad(gridDetail,'<s:message code="stat.detail.interest.list"/>');
		} else {
			csvDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_INTEREST"/>');
		}
	});
...
function getInterestUserOptions(){
	ui.get({
		...
		success : function(data, total) {
			...
			if(data.length>0) {
				$('#interGroup').val(data[0].groupSeq);
				$("#interGroup").change();
			}
...
function queryMakePop(  ){
	var url    = '<c:url value="/commons/queryMake.do?statType=interestUser"/>';
...
<body class="mini-navbar">
					...
					<div class="col-xs-12 text-left">
						<div class="form-group form-inline not-dashed">
							<div class="form-group">
								<label for="interGroup"><s:message code="condition.select.interest"/>:</label>
							...
							<div class="form-group" style="margin-left: 15px;">
								...
								<select id="xAxis" name="xAxis" class="input-sm form-control">
									...
									<option value="attachtype"><s:message code="condition.attach_type"/></option>
	...
	<script type="text/javascript">
		...
		grid1.loadExportMenu('<s:message code="DATA_ANALYSIS.STAT_INTEREST"/>');
		grid1.loadPageSize();
		...
		grid1.onClick = function() {
			...
			chartDat[tabID] = dat;
			printChart(dat);
			gridObj.loadExportMenu('<s:message code="stat.detail.interest.list"/>');
				...
				success : function(data, total) {
					...
					for ( var i=0 ; i < data.pivotHeader.length ; i++ ) {
						...
						if ( xAxis == "ctime_yyyymmdd") HeaderNm = Header.substr(0,4)+"-"+Header.substr(4,2)+"-"+Header.substr(6,2);
							...
							if(Header == "C") HeaderNm = '<s:message code="common.svc.community"/>';
							else if(Header == "E") HeaderNm = '<s:message code="common.svc.groupware"/>';
							else if(Header == "F") HeaderNm = '<s:message code="common.svc.filetrans"/>';
							else if(Header == "M") HeaderNm = '<s:message code="common.svc.mail"/>';
							else if(Header == "Q") HeaderNm = '<s:message code="common.svc.message"/>';
							else if(Header == "S") HeaderNm = '<s:message code="common.svc.social"/>';
							else if(Header == "U") HeaderNm = '<s:message code="common.svc.webservice"/>';
							else if(Header == "V") HeaderNm = '<s:message code="common.svc.image_streaming"/>';
							else if(Header == "W") HeaderNm = '<s:message code="common.svc.webmail"/>';
							else HeaderNm = '<s:message code="common.svc.etc"/>';
