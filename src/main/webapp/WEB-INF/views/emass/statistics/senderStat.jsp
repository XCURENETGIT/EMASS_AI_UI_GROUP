...
$(document).ready(function(){
	...
	$('.print_stat').click(function() {
		...
		if(gridDetail != undefined) {
			...
			gridDetail.print('<s:message code="stat.detail.sender.list"/>', pMenuId, menuId);
		} else {
			if (grid1.Rows == 0) {
				alert('<s:message code="common.msg.nodata"/>');
				return;
			}
			grid1.print('<s:message code="DATA_MONITOR.STAT_SENDER"/>', pMenuId, menuId);
		}
	});
	...
	$('.excel_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			excelDownLoad(gridDetail,'<s:message code="stat.detail.sender.list"/>');
		} else {
			chart = $('#chartArea1').highcharts();
			var svg = chart.getSVG();
			excelDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_SENDER"/>', svg);
		}
	});
	...
	$('.cell_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			cellDownLoad(gridDetail,'<s:message code="stat.detail.sender.list"/>');
		} else {
			cellDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_SENDER"/>');
		}
	});
	...
	$('.pdf_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			pdfDownLoad(gridDetail,'<s:message code="stat.detail.sender.list"/>');
		} else {
			pdfDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_SENDER"/>');
		}
	});
	...
	$('.csv_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			csvDownLoad(gridDetail,'<s:message code="stat.detail.sender.list"/>');
		} else {
			csvDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_SENDER"/>');
		}
	});
...
function queryMakePop(  ){
	var url    = '<c:url value="/commons/queryMake.do?statType=sender"/>';
...
<body class="mini-navbar">
						...
						<div class="form-group form-inline not-dashed">
							...
							<div class="form-group" style="margin-left: 15px;">
								...
								<select id="xAxis" name="xAxis" class="input-sm form-control">
									...
									<option value="direction_svc"><s:message code="condition.send"/></option>
	...
	<script type="text/javascript">
		...
		grid1.autoNumber();
		grid1.colAdd( "rowKey", '<s:message code="condition.sender"/>', 230, "left", false, 'link' );
		grid1.colAdd("total", '<s:message code="bodyview.total"/>', 130, "right", false, 'nomal' );
		grid1.loadExportMenu('<s:message code="DATA_ANALYSIS.STAT_SENDER"/>');
		grid1.loadPageSize();
		...
		grid1.onClick = function() {
			...
			chartDat[tabID] = dat;
			printChart(dat);
			gridObj.loadExportMenu('<s:message code="stat.detail.sender.list"/>');
