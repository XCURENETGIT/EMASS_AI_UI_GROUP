...
$(document).ready(function(){
	...
	$('.print_stat').click(function() {
		...
		if(gridDetail != undefined) {
			...
			gridDetail.print('<s:message code="stat.detail.url.list"/>', pMenuId, menuId);
		} else {
			if (grid1.Rows == 0) {
				alert('<s:message code="common.msg.nodata"/>');
				return;
			}
			grid1.print('<s:message code="DATA_MONITOR.STAT_URL"/>', pMenuId, menuId);
		}
	});
	...
	$('.excel_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			excelDownLoad(gridDetail,'<s:message code="stat.detail.url.list"/>');
		} else {
			chart = $('#chartArea1').highcharts();
			var svg = chart.getSVG();
			excelDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_URL"/>', svg);
		}
	});
	...
	$('.cell_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			cellDownLoad(gridDetail,'<s:message code="stat.detail.url.list"/>');
		} else {
			cellDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_URL"/>');
		}
	});
	...
	$('.pdf_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			pdfDownLoad(gridDetail,'<s:message code="stat.detail.url.list"/>');
		} else {
			pdfDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_URL"/>');
		}
	});
	...
	$('.csv_stat').click(function() {
		var gridDetail = getCurrentGrid();
		if(gridDetail != undefined) {
			csvDownLoad(gridDetail,'<s:message code="stat.detail.url.list"/>');
		} else {
			csvDownLoad(grid1,'<s:message code="DATA_MONITOR.STAT_URL"/>');
		}
	});
...
function queryMakePop(  ){
	var url    = '<c:url value="/commons/queryMake.do?statType=host"/>';
				...
				<div class="chartArea">
					<div>
						...
						<div class="sublist">
							...
							<div>
								<span class="tit">URL 총 합계</span>
								<p>99999<span class="text">건</span></p>
							</div>
							<div>
								<span class="tit">가장 많이 잡힌 시간</span>
								<p>99999<span class="text">건</span></p>
							</div>
							<div>
								<span class="tit">가장 적은 값</span>
								<p>99999<span class="text">건</span></p>
							</div>
						</div>
					</div>
...
<!--//Container-->
	...
	<script type="text/javascript">
		...
		grid1.loadExportMenu('<s:message code="DATA_ANALYSIS.STAT_URL"/>');
		grid1.loadPageSize();
